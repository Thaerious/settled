...
func _ready() -> void:
	...
	EventBus.set_house.connect(func(id:int, position: Axial): print("EventBus.set_house | id: %s | position: %s " % [id, position]))
	EventBus.add_resources.connect(func(id: int, resources: Array[Model.ResourceTypes]): print("EventBus.add_resources | id: %s | %s " % [id, resources]))
