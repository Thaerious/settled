...
func do_action(best: Array[Variant]) -> void:
	match best[1]:
		"house": 		
			EventBus.request_house.emit(self.id, Axial.from_key(best[2]))
		"city": 
			EventBus.request_city.emit(self.id, Axial.from_key(best[2]))
		"road": 
